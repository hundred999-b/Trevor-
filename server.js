...
app.post('/api/admin/login', (req, res) => {
  ...
  // DEBUG: Log credential comparison (mask password for security)
  console.log('[DEBUG] Admin login attempt:');
  console.log('  Received username:', JSON.stringify(username), 'length:', username.length);
  console.log('  Expected username:', JSON.stringify(ADMIN_USERNAME), 'length:', ADMIN_USERNAME.length);
...
app.get('/api/price', async (req, res) => {
  const now = Date.now();
  const baseCache = 5 * 60 * 1000;
  const backoffMultiplier = Math.min(priceCache.failedAttempts, 12);
  const CACHE_MS = baseCache + (backoffMultiplier * 5 * 60 * 1000);

  if (priceCache.lastFetch > 0 && (now - priceCache.lastFetch < CACHE_MS)) {
    return res.json({ price: priceCache.price, change: priceCache.change24h, cached: true, nextUpdateIn: Math.round((CACHE_MS - (now - priceCache.lastFetch)) / 1000) });
  }

  try {
    const response = await axios.get('https://api.coingecko.com/api/v3/simple/price?ids=hyperliquid&vs_currencies=usd&include_24hr_change=true', {
      timeout: 10000, headers: { 'Accept': 'application/json', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' }
    });
    const data = response.data.hyperliquid;
    priceCache = { price: data.usd, change24h: data.usd_24h_change || 0, lastFetch: now, failedAttempts: 0 };
    res.json({ price: priceCache.price, change: priceCache.change24h, cached: false });
  } catch (err) {
    ...
    res.json({ price: priceCache.price, change: priceCache.change24h, cached: true, stale: true, backoffMinutes: Math.round(CACHE_MS / 60000), error: err.message.includes('429') ? 'Rate limited by CoinGecko' : err.message });
  }
});
