...
// Fetch HYPE price from Hyperliquid native API (free, no rate limits)
async function fetchHypePrice() {
  try {
    ...
    // allMids returns an object with coin symbols as keys
    ...
    // Fallback: try spotMeta to get HYPE index
    ...
    // Find HYPE token index from spotMeta
    ...
    return { price: null, success: false, error: 'HYPE not found in market data' };
  } catch (err) {
    return { price: null, success: false, error: err.message };
  }
}
